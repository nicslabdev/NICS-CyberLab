...
const CUSTOM_DATASET_SELECTOR_UI = {
  ...
  loadBtn: document.getElementById("btn-load-custom-datasets"),
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadCustomDatasetsOnly() {
    const data = await jsonRequest(`${API_BASE}/profiles`, {}, "Loading custom datasets...");
  ...
  if (CUSTOM_DATASET_SELECTOR_UI.loadBtn) {
    CUSTOM_DATASET_SELECTOR_UI.loadBtn.addEventListener("click", async () => {
      try {
        await loadCustomDatasetsOnly();
      } catch (e) {
        setText("custom-dataset-output", { ok: false, error: e.message });
      }
    });
